...
export default function FloatingContactFab({ contact }) {
  ...
  const toggle = () => {
    setOpen((prev) => {
      if (!prev) trackEvent('floating_contact_open');
      return !prev;
    });
  };
  ...
  return (
    <aside className="floating-contact-fab" ref={rootRef} aria-label="Contact us">
      {open && (
        <div id={menuId} className="floating-contact-fab-menu" role="menu">
          <Link
            ...
            role="menuitem"
            data-track="click_floating_contact"
            onClick={close}
          >
            <ContactPageIcon />
            Contact page
      ...
      <button
        type="button"
        className="floating-contact-fab-trigger btn-conversion btn-conversion--inspection"
        aria-expanded={open}
        aria-haspopup="menu"
